...
|--------|---------|----------------------------|---------------|
| **Organization** | Tenant boundary | `id`, `name` | 1..* **User** |
| **User** | Login identity | `id`, `email`, `role` | belongs to **Organization** |
| **JobRequisition** | Hiring need | `id`, `title`, `status`, `headcount` | *posts* **JobPosting**; *has* **Application** |
| **JobPosting** | Channel-specific publication | `id`, `channel`, `status`, `url` | belongs to **JobRequisition** |
| **Candidate** | Person | `id`, `name`, `contact` | *submits* **Application** |
| **Application** | Candidate + job instance | `id`, `stage`, `submitted_at` | links **Candidate** ↔ **JobRequisition** |
| **AssessmentAttempt** | Test run | `id`, `status`, `score_summary` | belongs to **Application** |
| **Interview** | Scheduled event | `id`, `start_time`, `format` | belongs to **Application** |
| **Feedback** | Review notes/scorecard | `id`, `author`, `payload` | targets **Application** |
| **AutomationRule** | Trigger/action | `id`, `definition`, `enabled` | scoped to **Organization** |
| **AuditEvent** | Traceability | `id`, `action`, `actor`, `timestamp` | references domain records |

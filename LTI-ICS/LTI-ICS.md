...
| **Job posting** | Careers site + external channels, status, source attribution | FR-004–FR-006 |
| **Application intake** | Apply flow, resume storage, knockout questions, dedupe hooks | FR-007–FR-009 |
| **Application review** | Pipeline view (**owner** derived from hiring team; see Data model notes), scorecards, stage history, assistive AI | FR-010–FR-013 |
| **Online assessments** | Launch, provider handoff, outcome recording, restricted visibility | FR-014–FR-016 |
| **Interview scheduling** | Slots, calendar integration, notifications, interview feedback | FR-017–FR-019 |
| **Hire / reject** | Hired/rejected outcomes, templated comms, funnel metrics | FR-020–FR-022 |
| **Cross-cutting** | Comments and realtime updates, automations, AI toggles | FR-023–FR-028 |
...
sequenceDiagram
  ...
  participant BRD as Job board / careers channel
  ...
  WRK->>DB: Poll outbox
  WRK->>BRD: Publish job (HTTPS)
  BRD-->>WRK: Ack / posting ref
...
**Narrative:** The recruiter triggers an **Assessment attempt** from the pipeline; the **integration worker** (or API adapter) exchanges data with the **assessment provider**, then records **pass/fail/pending** on the attempt row. For interviews, the API proposes slots; **calendar provider** calls may be async. The candidate confirms; interviewers submit feedback linked to the **Interview** entity. **Hire** updates **Application** outcome and **Job requisition** fill counters, with audit entries.
...
flowchart LR
  ...
  EMAIL[Email provider]
  BRD[Job boards / careers endpoints]
  CAL[Calendar provider]
  ASMT[Assessment provider]
  LLM[LLM provider]
  REC --> SYS
...
- **Application and pipeline module:** intake, stages, history, **pipeline owner resolution** (FR-010: derived **`primary_recruiter`** on **`job_requisition_member`**, with documented fallbacks), **hire timestamps and start date**, requisition **fill counters**, metrics hooks (FR-007–FR-013, FR-020–FR-022).
- **Collaboration module:** comments and realtime event publish to **Redis** (FR-023–FR-024).
- **Automation engine module:** rule evaluation, scheduled triggers, append-only **`automation_run_log`** rows per run (rule id, actor/system, timestamp, outcome) (FR-025–FR-026).
- **AI orchestration module:** prompt build, provider call, **ai_inference_log** persistence (FR-012, FR-027–FR-028; ADR-005).
...
- **Pipeline owner (FR-010):** User shown as **owner** on an application in pipeline views—**derived** from **`job_requisition_member`** (**`primary_recruiter`** first), not a separate **`application`** column in MVP.  
- **Automation run log:** One row per automation **evaluation or execution**, for **FR-026** audit (rule, actor/system, time, outcome).  
